...
const validQuestion = {
  ...
  options: [{ content: 'A', id: 5371, letter: 'A', isAvailable: true }],
...
describe('can render /shop', () => {
  it('can click buy button', async () => {
    ...
    expect(mockPurchase).toHaveBeenCalledWith(expect.stringContaining('HAMMER'));

    // Buy 50-50 button
    await userEvent.click(buyButtons[1]!);
    expect(mockPurchase).toHaveBeenCalledWith(expect.stringContaining('FIFTY_FIFTY'));
    expect(mockPurchase).toHaveBeenCalledTimes(2);

    // Buy Swap button
    await userEvent.click(buyButtons[2]!);
    expect(mockPurchase).toHaveBeenCalledWith(expect.stringContaining('SWAP_QUESTION'));
